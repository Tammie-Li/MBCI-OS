...
import re
import serial
from shm import CreateShm,EEGTYPE,EEGMAXBYTES,EEGMAXLEN,MAXPOINTS,ACCTYPE,ACCMAXBYTES
import time
import math
import csv
from rdaclient import RDA
from adcdecoder import DevDecoder
...
class Device():
    def __init__(self,param,ctrprm):
        ...
        head = np.array((500, 8, 0), dtype=np.float32)
    ...
    def dataarange(self):
        ...
        print('-----------')
        print(dataay.shape[0]/8)
        print(len(self.accbytes)/6)
        ...
        self.shm.info[1] = curPoint + Leeg  # 下次写入的地方
        self.shm.info[2] = (curpakN + self.sampleCount * self.chs)
