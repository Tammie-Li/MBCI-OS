...
class EMGRecorder():
    def __init__(self, param, ctrprm, queue):
        '''
        e.g. param:
        ...
        e.g. ctrprm:
        '''
        super(EMGRecorder, self).__init__()
        self.daemon = True
    ...
    def update_trigger(self, trigger, block, speed):
    ...
    def dataarange(self):
        ...
        if self.saveFlg == 0:
            ...
            if self.shm.info[8] == 0:  # 结束保存
                ...
                blocks = np.array([self.blocks]).transpose()
                speeds = np.array([self.speeds]).transpose()


                ndata_col_ay = np.hstack((ndataay,accay,stamp, triggers, blocks, speeds))        # 将时间戳添加到最后一列
                print(ndata_col_ay.shape, blocks[-1], speeds[-1])
        ...
        if self.temporal_data is None: 
            ...
            if (self.temporal_data.shape[0] + self.sampleCount) < 50:
                self.temporal_data = np.concatenate((self.temporal_data, tmp_data))
            else:
                start = self.temporal_data.shape[0] + self.sampleCount - 50
                self.temporal_data = np.concatenate((self.temporal_data[start: ], tmp_data))

        if self.temporal_data.shape[0] == 50:
            self.queue.put(self.temporal_data)
...
def emg_recorder_target(parm, ctrparm, queue):
    emg_recorder = EMGRecorder(parm, ctrparm, queue)
    emg_recorder.run()
