...
class DataManager():
    ...
    # 依据时间长度等要求来确定数据的管理
    def config(self,srate,chs,period):
        self.dsN = srate//250
        self.dmL = 250*period
        self.chs = chs
        self.data = np.zeros((self.chs, self.dmL), dtype=EEGTYPE)
        self.packcache = np.zeros((self.chs,self.dsN))
        self.pack = None
        self.ptr = 0
        # 一般以20Hz速度更新
    ...
    def update(self,pack):
        ...
        if c < self.updateCof*self.dsN:   # 要求不少于20个数据点更新一次
            return 0
        ...
        if self.dsN == 1:
            ...
            pack = self.packcache[:,::self.dsN]
            self._update(pack[:,1:])
            idx = self.dsN * (math.ceil(c/self.dsN)-1)
            self.packcache = self.packcache[:,idx:]    #为了确保连续性，把截取之后的数据保留下来
            return 1
