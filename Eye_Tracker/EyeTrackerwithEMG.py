...
from main import EMGInterferce  # 导入肌电信号处理模块
import threading
...
GAZE_THRESHOLD = 0.0000000001  # 注视阈值
...
class GameState:
    def __init__(self):
        ...
        self.emg = EMGInterferce()  # 初始化肌电信号采集
        self.emg_lock = threading.Lock()  # 用于线程同步的锁
        self.emg_thread = threading.Thread(target=self.emg.start)
    ...
    def main_loop(self):
        ...
        while self.current_mode == 'game':
            ...
            for target in self.targets:
                ...
                with self.emg_lock:
                    emg_result = self.emg.result
                if recognized and emg_result == 1:  # 检查肌电信号是否为 1
